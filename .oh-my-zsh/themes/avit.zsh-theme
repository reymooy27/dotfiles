...
# settings
typeset +H _current_dir="%{$bg[black]%}%{$fg_bold[white]%}%3~%{$reset_color%}"
typeset +H _return_status="%{$fg_bold[red]%}%(?..⍉)%{$reset_color%}"
typeset +H _hist_no="%{$fg[grey]%}%h%{$reset_color%}"

PROMPT='
$(username) $(separator) ${_current_dir} $(separator) (git:$(git_prompt_info)) $(ruby_prompt_info)
%{%(!.${fg[red]}.${fg[white]})%}▶%{$reset_color%} '

PROMPT2='%{%(!.${fg[red]}.${fg[white]})%}◀%{$reset_color%} '

__RPROMPT='$(vi_mode_prompt_info)%{$(echotc UP 1)%} $(node_npm) $(git_prompt_status) ${_return_status}%{$(echotc DO 1)%}'
if [[ -z $RPROMPT ]]; then
  RPROMPT=$__RPROMPT
else
...
function node_npm() {
  ...
  echo "node: %{$fg[green]%}$nodeVersion%{$reset_color%} \u276f"
}
